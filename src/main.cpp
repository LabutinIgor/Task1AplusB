...
int main() {
    ...
    // По аналогии с заданием Example0EnumDevices узнайте какие есть устройства, и выберите из них какое-нибудь
    ...
    for (int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex) {
        ...
        OCL_SAFE_CALL(clGetDeviceInfo(devices[deviceIndex], CL_DEVICE_TYPE, deviceTypeSize, &deviceType,
                                      nullptr));
        if (deviceType == 4) {
    ...
    cl_int errcode;
    cl_context context = clCreateContext(nullptr, 1, &devices[deviceId], nullptr, nullptr, &errcode);
    ...
    cl_command_queue queue = clCreateCommandQueue(context, devices[deviceId], 0ull, &errcode);
    OCL_SAFE_CALL(errcode);
    ...
    cl_mem buf_as = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, n * sizeof(float), as.data(),
                                   &errcode);
    OCL_SAFE_CALL(errcode);

    cl_mem buf_bs = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, n * sizeof(float), bs.data(),
                                   &errcode);
    OCL_SAFE_CALL(errcode);

    cl_mem buf_cs = clCreateBuffer(context, CL_MEM_WRITE_ONLY, n * sizeof(float), nullptr, &errcode);
    OCL_SAFE_CALL(errcode);
...
//         std::cout << kernel_sources << std::endl;
    ...
    cl_program program = clCreateProgramWithSource(context, 1, &str, nullptr, &errcode);
    OCL_SAFE_CALL(errcode);
    ...
    cl_int build_err = clBuildProgram(program, 1, &devices[deviceId], "", nullptr, nullptr);

    size_t log_size = 0;
    OCL_SAFE_CALL(clGetProgramBuildInfo(program, devices[deviceId], CL_PROGRAM_BUILD_LOG, 0, nullptr, &log_size));
    std::vector<char> log(log_size, 0);
    OCL_SAFE_CALL(
            clGetProgramBuildInfo(program, devices[deviceId], CL_PROGRAM_BUILD_LOG, log_size, log.data(), nullptr));
    if (log_size > 1) {
    ...
    {
        unsigned int i = 0;
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(buf_as), &buf_as));
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(buf_bs), &buf_bs));
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(buf_cs), &buf_cs));
    ...
    //   - Дождаться завершения полунного события - см. в документации подходящий метод среди Event Objects
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            cl_event event;
            OCL_SAFE_CALL(
                    clEnqueueNDRangeKernel(queue, kernel, 1, nullptr, &global_work_size, &workGroupSize, 0, nullptr,
                                           &event));
        ...
        // - Среднее время выполнения кернела равно t.lapAvg() секунд
        std::cout << "VRAM bandwidth: " << 3 * n * sizeof(float) / 1024.0 / 1024 / 1024 / t.lapAvg() << " GB/s"
                  << std::endl;
    }
    ...
    // Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            OCL_SAFE_CALL(
                    clEnqueueReadBuffer(queue, buf_cs, CL_TRUE, 0, n * sizeof(float), cs.data(), 0, nullptr, nullptr));
            t.nextLap();
        }
        std::cout << "Result data transfer time: " << t.lapAvg() << "+-" << t.lapStd() << " s" << std::endl;
        std::cout << "VRAM -> RAM bandwidth: " << n * sizeof(float) / 1024.0 / 1024 / 1024 / t.lapAvg() << " GB/s"
                  << std::endl;
    }
    ...
    OCL_SAFE_CALL(clReleaseMemObject(buf_as));
    OCL_SAFE_CALL(clReleaseMemObject(buf_bs));
    OCL_SAFE_CALL(clReleaseMemObject(buf_cs));
